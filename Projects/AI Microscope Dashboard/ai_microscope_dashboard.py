...
@app.route('/microscope.jpg')
def microscope_image(request):
    return FileResponse('/home/mpcr/Desktop/microscope.jpg')
...
def htmldoc(content):
    return f'''
        <html>
            ...
            <body>
                <h1>Welcome to the AI Microscope Dashboard</h1>
                <div>
                    <ul>
...
@app.route('/')
def home(request):
    with open('/home/mpcr/Desktop/microscope.jpg', "rb") as img_file:
        encoded_string = base64.b64encode(img_file.read()).decode('utf-8')
    content = f'''
    <img src="data:image/jpg;base64,{encoded_string}" alt="Microscope Image" style="display: block; margin-left: auto; margin-right: auto; width: 100%;">
...
def capture_image(folder_name):
   ...
   # Define the directories to save images
    ...
    while True:
        ...
        # If 'd' is pressed on the keyboard, save the frame in directory1
        if key & 0xFF == ord('d'):
        ...
        # If 'f' is pressed on the keyboard, save the frame in directory2
        elif key & 0xFF == ord('f'):
    ...
    return f'<h2>Image captured successfully and saved in the respective folder.</h2>'
...
def microscope_connection(request):
    content = '''
        ...
        <p>Click the button below to capture an image from the microscope and save it in a specified folder.</p>
...
def label_data(request):
    ...
    # Save the labels to a DataFrame (could also be saved this to a file)
    global df
    df = pd.DataFrame({
...
def train_model(request):
    ...
    # Train the model
    history = model.fit(datagen.flow(X_train, y_train, batch_size=32), validation_data=(X_val, y_val), epochs=100)  # adjust the batch_size and epochs as needed
...
def save_model(request):
    global model
    model_name = 'my_model'  # Give a name to your model
    model_path = os.path.join('/home/mpcr/Desktop/save', model_name)  # Replace '/path/to/save' with the actual path where you want to save the model
    model.save(model_path)

    return htmldoc(f'<h2>Successfully saved the model.</h2>')

#======================================================================================================================================

@app.route('/load-model')
def load_model_route(request):
    global model
    model_path = '/home/mpcr/Desktop/save/my_model'  # Replace '/path/to/save' with the actual path where you saved the model
    model = load_model(model_path)

    return htmldoc(f'<h2>Successfully loaded the model.</h2>')
...
def test_model(request):
    ...
    return htmldoc(f'<h2>Accuracy of the model on test images:</h2><h2>{100 * accuracy}%</h2>')
...
def run_model(request):
    # Load an image (you would typically get this image from your microscope in a real use-case)
    img_path = '/home/mpcr/Desktop/images/train/class_1/frame_0.png'  # replace with the path to the image you want to classify
    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)  # read in grayscale, use cv2.IMREAD_COLOR for color images
    img = cv2.resize(img, (224, 224))  # resize to the size of the images the model was trained on
    img_normalized = img / 255  # normalize the pixel values to the range [0, 1]

    # Add an extra dimension for the batch size and number of channels
    ...
    return htmldoc(f'<h2>Prediction:</h2><p>The model predicts that the image belongs to class {predicted_class}.</p>')
#======================================================================================================================================
...
def visualize(request):
    try:
        # Load an image
        img_path = '/home/mpcr/Desktop/images/train/class_1/frame_0.png'  
        encoded_string = convert_img_to_base64(img_path)

        html = f"""
        <h2>Image:</h2>
        <img src="data:image/jpeg;base64,{encoded_string}" alt="Image">
        """
        return htmldoc(html)

    except Exception as e:
        return htmldoc(f'<h2>Error:</h2><p>{str(e)}</p>')
